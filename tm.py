...
threshold = 0.5 #sebagai pengatur kepekaan pencocokan gambar
loc = np.where( res >= threshold) #pencocokan gambar utama dan wajah

for pt in zip(*loc[::-1]):
    cv2.rectangle(img_rgb, pt, (pt[0] + w, pt[1] + h), (0,255,0), 2) #menampilkan marking berupa kotak pada wajah yang dianggap sama atau cocok
